...
Port::Port(Handle& hcan, std::vector<Filter> filters, uint8_t slave_start)
    : hcan_(hcan), slave_start_(slave_start) {
  filters_ = new std::vector(filters);
}

void Port::awake() {
  if (filters_->empty()) filters_->push_back(Filter());
  ...
  if (FIFO1_enable) {
    if (HAL_CAN_ActivateNotification(&hcan_, CAN_IT_RX_FIFO1_MSG_PENDING) !=
        HAL_OK) {
      Error_Handler();
    }
...
bool Port::transmit(const CAN_TxHeaderTypeDef* header, const uint8_t* data,
                    uint32_t* mail_box) const {
...
void Port::add_std_callback(uint32_t frame_id,
                            std::function<void(const RcvData&)> callback) {
  std_callbacks_[frame_id] = callback;
}

void Port::add_ext_callback(uint32_t frame_id,
                            std::function<void(const RcvData&)> callback) {
  ext_callbacks_[frame_id] = callback;
}

void Port::exec_callback(const CAN_RxHeaderTypeDef& frame_header,
...
void Port::notify_fifo1(CAN_HandleTypeDef* hcan,
                        const CAN_RxHeaderTypeDef& header,
                        const uint8_t* data) {
