...
PortBase::PortBase(Handle& huart, bool idle, uint8_t* buffer, uint32_t size)
    : huart_(huart), buffer_(buffer), size_(size), half_(size / 2U) {
      status_.idle = idle;
      ports_.emplace_back(this);
  }
...
void PortBase::awake() {
  check_status();
  if (status_.dma.tx.enable) {
    tx_ = &dma_tx;
  } else if (status_.interrupt) {
    tx_ = &it_tx;
  } else {
    tx_ = &bl_tx;
  }

  if (status_.interrupt) {
    if (!status_.dma.rx.enable) {
      if (!status_.idle) launch_rx_ = HAL_UART_Receive_IT;
      else launch_rx_ = HAL_UARTEx_ReceiveToIdle_IT;
    } else {
      if (!status_.idle) launch_rx_ = HAL_UART_Receive_DMA;
      else launch_rx_ = HAL_UARTEx_ReceiveToIdle_DMA;
    }
  }
...
bool PortBase::transmit(const uint8_t* data, size_t len) const {
  return tx_(&huart_, data, len);
}

bool PortBase::transmit_blocking(const uint8_t* data, size_t len,
                                 uint32_t timeout) const {
  return HAL_UART_Transmit(&huart_, data, len, timeout) == HAL_OK;
}

void PortBase::add_tx_callback(Callback callback) {
  tx_callbacks_.emplace_back(callback);
}

void PortBase::add_rx_callback(Callback callback) {
  rx_callbacks_.emplace_back(callback);
}

void PortBase::check_status() {
  uint32_t cr1 = READ_REG(huart_.Instance->CR1);
  IRQn_Type irqn = get_uart_irqn(huart_);

  // clang-format off
  status_.enable          = HAL_IS_BIT_SET(cr1, USART_CR1_UE);
  status_.interrupt       = NVIC_GetEnableIRQ(irqn);
  // status_.idle            = huart_.ReceptionType == HAL_UART_RECEPTION_TOIDLE;
  status_.dma.tx.enable   = huart_.hdmatx != nullptr;
  status_.dma.tx.circular = huart_.hdmatx->Init.Mode == DMA_CIRCULAR;
  status_.dma.rx.enable   = huart_.hdmarx != nullptr;
  status_.dma.rx.circular = huart_.hdmarx->Init.Mode == DMA_CIRCULAR;
  // clang-format on
}

IRQn_Type PortBase::get_uart_irqn(const Handle& huart) const {
#ifdef USART1
  if (huart.Instance == USART1) return USART1_IRQn;
#endif
#ifdef USART2
  if (huart.Instance == USART2) return USART2_IRQn;
#endif
#ifdef USART3
...
/*
  It only happens when the IDLE interrupt is enabled and the DMA is in the
  CIRCULAR mode. It is expected that the duration of IDLE is longer than the
  execution of this function, otherwise you might lose incoming data. However,
  the special case above can hardly happen in the normal situation.
*/
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef* huart) {
  vtb::uart::PortBase::notify_tx(huart);
}
